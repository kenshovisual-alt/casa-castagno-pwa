...
export default function Providers() {
  ...
  const del = (provider) => {
    ...
    deleteWithUndo({ resource: "providers", record: provider, label: "Contact", onSettled: load });
  };
    ...
    <div className="px-6 md:px-10 lg:px-14 py-10 max-w-[1400px]">
      ...
      {filtered.length === 0 ? (
        <EmptyState
          ...
          hint={items.length === 0 ? "Add chefs, cleaners, drivers and other partners you work with." : "No contacts match your search or filter."}
          action={items.length === 0 && (
            <Link to="/contacts/new" className="cc-btn-primary inline-flex items-center gap-2">
              <Plus size={16} /> New contact
            </Link>
          )}
        />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((p) => (
            <div key={p.id} className="cc-card p-5" data-testid={`prov-card-${p.id}`}>
              <div className="cc-overline mb-2">{PROVIDER_TYPE_LABEL[p.type]}</div>
              ...
              <div className="mt-3 flex justify-end gap-2">
                ...
                <button className="cc-btn-ghost" onClick={() => del(p)} data-testid={`prov-del-${p.id}`}><Trash2 size={14} /></button>
              </div>
            </div>
          ))}
        </div>

...
export default function BookingForm() {
  ...
  useEffect(() => {
    if (id) api.get("bookings", id).then((d) => setB({ ...empty, ...d }));
  }, [id]);
    ...
    <form onSubmit={onSubmit} className="px-6 md:px-10 lg:px-14 py-10 max-w-[1100px]" data-testid="booking-form">
      ...
      <section className="cc-card p-6 mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <Field label="Adults"><input className="cc-input" type="number" min="0" value={b.adults} onChange={(e) => set("adults", e.target.value)} data-testid="f-adults" /></Field>
          <Field label="Children"><input className="cc-input" type="number" min="0" value={b.children} onChange={(e) => set("children", e.target.value)} data-testid="f-children" /></Field>
        ...
        <div className="mt-3 text-xs" style={{ color: "var(--cc-muted)" }}>Nights: <strong style={{ color: "var(--cc-forest)" }}>{nights}</strong> · Total guests: <strong style={{ color: "var(--cc-forest)" }}>{Number(b.adults) + Number(b.children)}</strong></div>
      </section>
      ...
      <section className="cc-card p-6 mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Field label="Gross amount"><input className="cc-input" type="number" value={b.gross_amount} onChange={(e) => set("gross_amount", e.target.value)} data-testid="f-gross" /></Field>
          ...
          <Field label={`Commission % ${isDirect ? "(direct: 0)" : ""}`}>
            <select className="cc-input" value={b.commission_pct} onChange={(e) => set("commission_pct", e.target.value)} disabled={isDirect} data-testid="f-commission">
          ...
          <Field label="Deposit amount"><input className="cc-input" type="number" value={b.deposit_amount} onChange={(e) => set("deposit_amount", e.target.value)} data-testid="f-deposit" /></Field>
          <Field label="Cleaning fee"><input className="cc-input" type="number" value={b.cleaning_fee} onChange={(e) => set("cleaning_fee", e.target.value)} /></Field>
          <Field label="Extras revenue"><input className="cc-input" type="number" value={b.extras_revenue} onChange={(e) => set("extras_revenue", e.target.value)} /></Field>
          <Field label="Experience revenue"><input className="cc-input" type="number" value={b.experience_revenue} onChange={(e) => set("experience_revenue", e.target.value)} /></Field>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
          <div><div className="overline">Commission</div><div className="serif text-xl" style={{ color: "var(--cc-terracotta)" }}>{formatMoney(fin.commission, b.currency)}</div></div>
          <div><div className="overline">Net owner</div><div className="serif text-xl" style={{ color: "var(--cc-forest)" }}>{formatMoney(fin.net, b.currency)}</div></div>
          <div><div className="overline">Balance due</div><div className="serif text-xl" style={{ color: "var(--cc-forest)" }}>{formatMoney(fin.balance, b.currency)}</div></div>
          <div><div className="overline">Total revenue</div><div className="serif text-xl" style={{ color: "var(--cc-forest)" }}>{formatMoney(fin.total, b.currency)}</div></div>
        </div>
      </section>
